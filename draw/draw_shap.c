...
/*   Updated: 2022/12/04 18:45:08 by ykhadiri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	circle(t_mlx *mlx, int x, int y, int r, int color)
{
	...
	while (r)
	{
		...
		while(i < 360)
		{
			...
			my_mlx_pixel_put(&mlx->image, (x + x1), (y + y1), color);
			i += 0.1;
		}
		r--;
	}
...
{
	int x_end = data->player.cord.x + (cos(data->player.rotation_angle) * 20);
	int y_end = data->player.cord.y + (sin(data->player.rotation_angle) * 20);
	circle(data->mlx, data->player.cord.x ,data->player.cord.y, SIZE_CUB / 4, RED);
